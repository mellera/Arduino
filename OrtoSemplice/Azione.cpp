...
class Azione {
  public:
    void set(int _comando, uint32_t _durata_acceso, uint32_t _periodo_ripetizione, uint32_t _prossima_esecuzione, int _stato, int _pin_elettrovalvole[]) { // Setta i parametri di un'azione
      ...
      setStato(_stato);
      for(int i = 0; i < N_ELETTROVALVOLE; i++) {
    ...
    void esegui(uint32_t _now) {
      if (getStato() != SOVRASCRIVIBILE) {
        ...
        if((getProssimaEsecuzione() <= _now) && (getProssimaEsecuzione() > (_now - PERIODO_ESECUZIONE_ROUTINE))) {
    ...
    void controllaElettrovalvole(int _state) {
      for (int i = 0; i < N_ELETTROVALVOLE; i++) { // Ciclo sulle elettrovalvole collegate all'azione e setto uno stato alle uscite
    ...
    void setPeriodoRipetizione(uint32_t _periodo_ripetizione) {
      periodo_ripetizione = _periodo_ripetizione;
    }

    void setDurataAcceso(uint32_t _durata_acceso) {
    ...
    void setProssimaEsecuzione(uint32_t _prossima_esecuzione) {
    ...
    uint32_t getProssimaEsecuzione() {
    ...
    uint32_t getPeriodoRipetizione() {
      return periodo_ripetizione;
    }

    uint32_t getDurataAcceso() {
  ...
  private:
    ...
    int pin_elettrovalvole[N_ELETTROVALVOLE] = {-1};
