...
void MOTORE::inizia(int ora,int an,int ena)
{
     ...
     //la terza variabile gestisce la velocità
     enablePin=ena;
...
//gira in un senso alla velocità vel 
...
//gira nell'altro senso alla velocità vel
...
/*
passando un valore da -255 a 255 muove il motore "in avanti" di velocità vel se 
vel è positivo altrimenti va "all'indietro" di velocità vel
*/
void MOTORE::muovi(int vel)
{
     //se vel è maggiore di 0 allora fa girare il motore "in avanti" di velocità vel
     if(vel>0)
     {
          avanti(vel);
          return;
     ...
     //se vel è minore di 0 allora fa girare il motore "all'indietro" di velocità vel
     if(vel<0)
     {
          vel=vel*(-1);    
          indietro(vel);
     ...
     //se il parametro vel è uguale a zero il motore si ferma
     if(vel==0)
     {
          freno();
          return;
